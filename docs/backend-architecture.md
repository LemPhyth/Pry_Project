...
已迁移会话、消息、文件夹、长期记忆、聊天回合、角色、贴纸、外观媒体及模型配置的 HTTP 边界，保留原数据库格式，因而不需要数据迁移。模型进程注册表会按模型及其运行参数复用实例；文字和视觉选择相同模型时不会重复加载。桌面端不再直接写 SQLite、角色/贴纸清单或偏好配置；当前仍保留对旧配置文件的只读投影，后续将由 API 查询替代。
...
2. 使用已建立的 `Pry.Client` 让桌面窗口订阅 SSE，并调用角色、偏好、贴纸、模型选择、媒体及语音 API，只保留视图、录音采集、输入、导航和窗口生命周期。
3. 删除 `MainWindow` 中已经无调用方的数据库、配置文件和模型编排代码。
4. 删除迁移后无调用方的旧窗口业务代码，并执行完整 UI 回归。
