...
已迁移会话、消息、文件夹、长期记忆以及聊天回合的 HTTP 边界，保留原数据库格式，因而不需要数据迁移。模型进程注册表会按模型及其运行参数复用实例；文字和视觉选择相同模型时不会重复加载。桌面应用仍可运行，但迁移期间不要同时通过桌面进程和 API 修改同一会话；SQLite 能串行化写入，却不能协调两套内存中的回复状态。
